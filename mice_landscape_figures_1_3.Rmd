...
load_pkg('seqinr')
load_pkg('devtools')
#aload_pkg('robCompositions')
load_pkg('VennDiagram')
load_pkg('circlize')
...
names(data) <- sapply(strsplit(files, split = '[/_.]'), function(x) x[2])
...
#statistical testing:
solo_df$cohort <- limma::strsplit2(solo_df$id, split = "-")[,1]
...
annotation <- data.frame(limma::strsplit2(colnames(Vgene_mat_counts), split = "_"), row.names = colnames(Vgene_mat_counts), stringsAsFactors = T)
...
annotation <- data.frame(limma::strsplit2(colnames(cosine_all), split = "_"), row.names = colnames(cosine_all), stringsAsFactors = T)
colnames(annotation) <- c("Mouse", "Organ", "Cohort")
