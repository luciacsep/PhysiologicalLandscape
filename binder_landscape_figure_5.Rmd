...
names(yeast) <- sapply(strsplit(files, split = '[/_.]'), function(x) x[2])
...
names(data) <- sapply(strsplit(files[19:36], split = '[/_.]'), function(x) x[2])
...
ggplot(df_bind_no0[[m]], aes(x = Count, y = MAF_Clonal_.)) +
  geom_point(all_CDR3_no0[[m]], mapping = aes(x = Count, y = MAF_Clonal_.), color="grey", alpha = 0.5, position = "jitter") +
  geom_boxplot(width = 0.9) +
  stat_summary(all_CDR3_no0[[m]], mapping = aes(x = Count, y = MAF_Clonal_.),fun.y = median, fun.ymin = median, fun.ymax = median, geom = "crossbar", color = '#737373', size = 0.4) +
  ...
  ylab("Frequency in log10") +
  scale_x_discrete(labels=c("1", "2", "3", "4", "5", "6"), position = "bottom") +
  facet_wrap( ~ Count, ncol=6, scales = "free_x") + theme_cowplot() + theme(axis.title.y = element_text(size = 16),  axis.ticks.x = element_blank(), axis.title = element_text(size = 16), axis.text = element_text(size = 16), strip.text = element_text(size=14), legend.title = element_text(size = 16, face = "bold"), legend.text = element_text(size = 16),  strip.background = element_blank(), strip.text.x = element_blank()) 
 setwd(fpath)
}
